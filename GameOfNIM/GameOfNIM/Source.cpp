...
void nim()
{
    ...
    cout << "Introdueix un numero pel qual comencar (entre 0 i 50): ";  // Demana el número inicial

    // Comprova que el número inicial estigui entre 0 i 50
    while (true) {
        cin >> numInicial;
        if (numInicial >= 0 && numInicial <= 50) {
            break;  // Surt del bucle si el número és vàlid
        }
        else {
            cout << "Numero incorrecte. Introdueix un numero entre 0 i 50: ";  // Si no és vàlid, torna a demanar-lo
        }
    }
